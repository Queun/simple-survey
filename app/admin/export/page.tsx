  ...
  async function handleExport(format: 'excel' | 'csv') {
    ...
    try {
      ...
      const url = selectedPeriod
        ? `/api/admin/export?format=${format}&periodId=${selectedPeriod}`
      ...
      let filename = `survey-data-${Date.now()}.${format === 'excel' ? 'xlsx' : 'csv'}`
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">导出调研数据为 Excel 或 CSV 格式</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 mb-2">格式说明：</h3>
                <div className="space-y-2 text-gray-600">
                  <div>
                    <span className="font-medium">Excel (.xlsx)：</span>
                    <p className="ml-4">适合在 Microsoft Excel、WPS 表格等软件中打开，支持格式化和公式</p>
                  </div>
                  <div>
                    <span className="font-medium">CSV (.csv)：</span>
                    <p className="ml-4">纯文本格式，可用任何文本编辑器或表格软件打开，兼容性最好</p>
                  </div>
                </div>
              </div>

              <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-3 py-2 rounded text-xs">
                <strong>注意：</strong>导出的数据可能包含敏感信息，请妥善保管导出的文件。
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-orange-50 p-4 rounded-lg">
                <div className="text-sm text-gray-600">导出格式</div>
                <div className="text-lg font-bold text-orange-600">Excel / CSV</div>
              </div>
            </div>
          </CardContent>
        </Card>
