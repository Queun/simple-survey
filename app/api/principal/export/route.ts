...
// GET /api/principal/export - 导出本校统计数据
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      const whereCondition: any = {
        schoolId: user.schoolId // 只导出本校数据
      }
      if (periodId) {
      ...
      if (format === 'csv') {
        ...
        return new Response(csv, {
          ...
          headers: {
            ...
            'Content-Disposition': `attachment; filename="school-survey-data-${Date.now()}.csv"`,
          },
        })
      } else {
        // 生成Excel
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, '本校调研数据')
        ...
        return new Response(excelBuffer, {
          ...
          headers: {
            ...
            'Content-Disposition': `attachment; filename="school-survey-data-${Date.now()}.xlsx"`,
          },
        })
      }
    } catch (error) {
